...
static int mt_spi_setup(struct spi_device *spidev)
{
	...
	if (!chip_config) {
		...
		SPI_DBG("%s spi max_speed_hz(%lu)\n", __func__, spidev->max_speed_hz);
		switch(spidev->max_speed_hz)
		{
		case 34000000:
			chip_config->high_time = 2;
		...
		chip_config->cpol = (spidev->mode & SPI_CPOL);
		chip_config->cpha = (spidev->mode & SPI_CPHA);
		SPI_DBG("%s cpol(%d), cpha(%d)\n", __func__, chip_config->cpol, chip_config->cpha);
	...
	SPI_DBG("set up chip config,mode:%d\n", chip_config->com_mod);
