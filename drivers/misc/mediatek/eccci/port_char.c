...
static void dev_char_open_check(struct ccci_port *port)
{
	...
	if (port->rx_ch == CCCI_MD_LOG_RX)
		port->modem->critical_user_active[2] = 0; /* ttyC1 */
	if (port->rx_ch == CCCI_UART1_RX)
		port->modem->critical_user_active[3] = 0; /* MD logger control */
}
...
static ssize_t dev_char_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (port->modem->md_state == EXCEPTION && port->tx_ch != CCCI_MD_LOG_TX && port->tx_ch != CCCI_UART1_TX
		...
		CCCI_ERR_MSG(port->modem->index, CHAR, "md state=%d, tx_ch=%d, name=%s\n", port->modem->md_state, port->tx_ch, port->name);
		return -ETXTBSY;
	}
...
void drv_tri_panic_by_lvl(void *ctl)
{
	struct ccci_modem *md = (struct ccci_modem *)ctl;

	if (ccci_md_log_level == MD_LOG_LEVEL_LOW) {
		/* modem reset */
		CCCI_INF_MSG(md->index, CHAR, " Curr log level is low, trigger reset\n");
		ccci_trigger_reset(ctl);
	} else if ((ccci_md_log_level == MD_LOG_LEVEL_MID) || (ccci_md_log_level == MD_LOG_LEVEL_HIGH)) {
		/* call panic to trigger upload */
		CCCI_INF_MSG(md->index, CHAR, " call panic to trigger upload\n");
		panic("CP Crash");
	} else
		CCCI_INF_MSG(md->index, CHAR, " md log level is invalid, do nothing\n");
}
...
static long dev_char_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CCCI_IOC_GET_MD_IMG_EXIST:
		...
		CCCI_INF_MSG(md->index, CHAR, "CCCI_IOC_GET_MD_IMG_EXIST: lk md_type: %d\n", md_type);
		if (md_type) {
	...
	case CCCI_IOC_ENTER_UPLOAD2:
		...
		case MD_EX_TYPE_ASSERT:
			snprintf(ex_info, EE_BUF_LEN, "CP Crash\n[%s] file:%s line:%d\np1:0x%08x\np2:0x%08x\np3:0x%08x\n",
				 debug_info->name,
...
cp_crash:
	...
	case CCCI_IOC_DRV_ENTER_UPLOAD:
		ccci_drv_trigger_upload = 1;
		CCCI_INF_MSG(md->index, CHAR, "drv trigger upload called by %s\n", current->comm);
		break;

	case CCCI_IOC_LOG_LVL:
		if (copy_from_user(&ccci_md_log_level, (void __user *)arg, sizeof(unsigned int))) {
			CCCI_INF_MSG(md->index, CHAR, "cpy log level fail: copy_from_user fail!\n");
			ret = -EFAULT;
		}
		CCCI_INF_MSG(md->index, CHAR, "cpy log level value:0x%x\n", ccci_md_log_level);
		break;

	case CCCI_IOC_FORCE_MD_OFF:
		ret = md->ops->force_off(md);
		break;
...
static void port_char_md_state_notice(struct ccci_port *port, MD_STATE state)
{
	...
	if ((port->rx_ch == CCCI_RIL_IPC0_RX || port->rx_ch == CCCI_RIL_IPC1_RX) && 
		(state == GATED || state == BOOTING || state == READY || state == RESET || state == EXCEPTION)) {
		wake_up_all(&port->rx_wq);
