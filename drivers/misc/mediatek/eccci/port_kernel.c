...
#endif
	case MD_DISPLAY_DYNAMIC_MIPI_2G:
	case MD_DISPLAY_DYNAMIC_MIPI_3G:
	case MD_DISPLAY_DYNAMIC_MIPI_4G:
	case MD_THERMAL_LOW_POWER:
...
static int ccci_rpc_remap_ch_queue(struct ccci_modem *md, struct ccci_rpc_queue_mapping *map_tbl)
{
	unsigned int i, j = 0;
	struct ccci_port *port;

	if (map_tbl->ch_cnt > MAX_CHANNEL_QUEUE_MAPPING) {
		CCCI_ERR_MSG(md->index, RPC, "map table: invalid channel count %u\n", map_tbl->ch_cnt);
		return -1;
	}

	for (i = 0; i < map_tbl->ch_cnt; i++) {
		CCCI_INF_MSG(md->index, RPC, "rpc data: ch %u to q%u\n",
		...
		if (map_tbl->ch_queue_mapping[i].channel >= CCCI_MAX_CH_NUM ||
			...
			CCCI_ERR_MSG(md->index, RPC, "map table: item %d is invalid: ch %u to q%u\n", i,
				map_tbl->ch_queue_mapping[i].channel, map_tbl->ch_queue_mapping[i].queue_type);
			return -1;
		}
		for (j = 0; j < md->port_number; j++) {
			port = md->ports + j;
			if (port->tx_ch == map_tbl->ch_queue_mapping[i].channel) {
				CCCI_INF_MSG(md->index, RPC, "remap ch %d from q%u to q%u\n", port->tx_ch,
			...
			if (port->dl_ack_ch == map_tbl->ch_queue_mapping[i].channel) {
				CCCI_INF_MSG(md->index, RPC, "remap ch %d from q 0x%x to 0x%x\n", port->dl_ack_ch,
	...
	for (j = 0; j < md->port_number; j++) {
		port = md->ports + j;
		CCCI_DBG_MSG(md->index, RPC, "tx[%u],ack[%u]:[%u],[%u]\n", port->tx_ch, port->dl_ack_ch,
	...
	case IPC_RPC_CCCI_QUEUE_MAPPING:
		{
			...
			CCCI_INF_MSG(md->index, RPC, "op_id[0x%X]: pkt_num=%d, pkt[0] len %u!\n",
				     p_rpc_buf->op_id, pkt_num, pkt[0].len);

			map_tbl = (struct ccci_rpc_queue_mapping *)(pkt[0].buf);
			ccci_rpc_remap_ch_queue(md, map_tbl);
			pkt_num = 0;
			tmp_data[0] = 0;
			pkt[pkt_num].len = sizeof(unsigned int);
			pkt[pkt_num++].buf = (void *)&tmp_data[0];
...
void md_ex_monitor2_func(unsigned long data)
{
	...
	if (ccci_drv_trigger_upload) {
		pr_err("[md1] drv trigger panic\n");
		drv_tri_panic_by_lvl(md);
		return;
	}

	if (!is_normal_ee_case) {
		is_normal_ee_case = 0;
		pr_err("[md1] EE time out case, call panic\n");
		drv_tri_panic_by_lvl(md);
	}
}
