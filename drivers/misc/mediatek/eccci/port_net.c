...
#define GET_CCMNI_IDX(p) (((p)->minor & 0xF) - 1)

#ifdef CCMNI_U

static inline struct ccci_port *ccmni_get_port_by_id(struct ccci_modem *md, int ccmni_idx)
{
	int i = 0;
	struct ccci_port *port = NULL;

	while (i < md->port_number) {
		port = md->ports + i;
		if (port->minor == (0xF0 | (ccmni_idx + 1)))
			return port;
		i++;
	}
	return NULL;
...
static int ccmni_get_default_channel(int md_id, int ccmni_idx, struct ccmni_ch *channel)
{
	int ret = 0;
...
int ccci_get_ccmni_channel(int md_id, int ccmni_idx, struct ccmni_ch *channel)
{
	...
	if (md == NULL) {
		...
		ret = ccmni_get_default_channel(md_id, ccmni_idx, channel);
		return ret;
	}

	if (ccmni_idx >= md->port_number) {
		CCCI_ERR_MSG(md_id, NET, "invalid ccmni_idx %d for md_id %d !!\n", ccmni_idx, md_id);
		return -1;
	}

	port = ccmni_get_port_by_id(md, ccmni_idx);
	if (port) {
...
static int is_ack_tx_channel(int tx_ch, int *pch_to_port)
{
	if (tx_ch == CCCI_CCMNI1_DL_ACK)
		*pch_to_port = CCCI_CCMNI1_TX;
	else if (tx_ch == CCCI_CCMNI2_DL_ACK)
		*pch_to_port = CCCI_CCMNI2_TX;
	else if (tx_ch == CCCI_CCMNI3_DL_ACK)
		*pch_to_port = CCCI_CCMNI3_TX;
	else if (tx_ch == CCCI_CCMNI4_DLACK_RX)
		*pch_to_port = CCCI_CCMNI4_TX;
	else if (tx_ch == CCCI_CCMNI5_DLACK_RX)
		*pch_to_port = CCCI_CCMNI5_TX;
	else if (tx_ch == CCCI_CCMNI6_DLACK_RX)
		*pch_to_port = CCCI_CCMNI6_TX;
	else if (tx_ch == CCCI_CCMNI7_DLACK_RX)
		*pch_to_port = CCCI_CCMNI7_TX;
	else if (tx_ch == CCCI_CCMNI8_DLACK_RX)
		*pch_to_port = CCCI_CCMNI8_TX;
	else
		*pch_to_port = tx_ch;

	return !(*pch_to_port == tx_ch);
}
...
int ccmni_send_pkt(int md_id, int tx_ch, void *data)
{
	...
	int tx_ch_to_port, tx_queue, is_ack;
	int ret;
#ifdef PORT_NET_TRACE
	...
	tx_ch_to_port = -1;
	is_ack = is_ack_tx_channel(tx_ch, &tx_ch_to_port);
...
static inline void init_dl_ack_ch(struct ccci_port *port)
{
	...
	case 7:
		...
		break;
	default:
		CCCI_ERR_MSG(port->modem->index, NET, "invalid ccmni index=0x%x\n", port->minor);
		break;
	}
}
...
#endif
	ccmni_ops.rx_callback(port->modem->index, GET_CCMNI_IDX(port), skb, NULL);
#ifdef PORT_NET_TRACE
...
static void port_net_md_state_notice(struct ccci_port *port, MD_STATE state)
{
	...
	if (((state == TX_IRQ) && ((port->flags & PORT_F_RX_FULLED) == 0)) ||
		...
		return;
	ccmni_ops.md_state_callback(port->modem->index, GET_CCMNI_IDX(port), state,
...
void port_net_md_dump_info(struct ccci_port *port, unsigned int flag)
{
	...
	ccmni_ops.dump(port->modem->index, GET_CCMNI_IDX(port), 0);
#endif
}
