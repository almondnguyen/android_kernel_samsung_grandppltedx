...
int do_ptim(bool isSuspend)
{
	...
	int ret = 0;
#if defined(CONFIG_MTK_SMART_BATTERY) && defined(CONFIG_MTK_TC10_FEATURE)
...
PMIC_RG_AUXADC_SMPS_CK_PDN_HWEN));*/
	...
	psy_do_property("battery", get, POWER_SUPPLY_PROP_CURRENT_NOW, val);
	ptim_R_curr = val.intval * 10;
	if (ptim_R_curr < 0)
		ptim_R_curr = 0 - ptim_R_curr;
...
#ifdef CONFIG_SEC_PM
				if(pmic_suspend_state && last_resume_kernel_reason_suspend) {
					last_resume_kernel_reason_len +=
						snprintf(last_resume_kernel_reason + last_resume_kernel_reason_len,
						MAX_WAKEUP_REASON_LEN - last_resume_kernel_reason_len,
						"[PMIC]%s|", interrupts[i].interrupts[j].name);
				}
#endif
...
static int pmic_thread_kthread(void *x)
{
	...
	while (1) {
		...
		mt_eint_unmask(g_eint_pmic_num);
#else
		if (g_pmic_irq != 0 && register_int_state == 0)
			enable_irq(g_pmic_irq);
#endif
		schedule();
	}
...
unsigned short pmic_is_battery_plugout(void)
{
	...
	int uvlo_rstb_status = g_uvlo_status;
	...
	PMICLOG("[pmic_is_battery_plugout] [%d] %d, %d, %d, %d\n", is_battery_plugout,
...
void PMIC_INIT_SETTING_V1(void)
{
	...
	if (!strcmp(CONFIG_ARCH_MTK_PROJECT, "titan6737t_rev00_m"))
		is_rev00 = 1;
#endif
...
#if defined(CONFIG_MTK_TC10_FEATURE)
	if (!is_rev00)
		ret = pmic_config_interface(0xCA4, 0x0, 0x1, 0); /* [0:0]: FG_ON */

	if(board_id > 2)  //Apply to over HW rev 0.2
		ret = pmic_config_interface(0x63E, 0x3C, 0x7F, 0);
		/*[6:0]: VSYS22_VOSEL; 08/19 for Titan low request, change Vout from2.0 to 1.95.*/
#endif
}
...
static int pmic_mt_probe(struct platform_device *dev)
{
	...
	int len = 0;
#endif
#ifdef CONFIG_PMIC_BATTERY_THERMAL_DISABLE
	u32 rVal;
#endif
#if defined(CONFIG_MTK_SMART_BATTERY)
	...
	if (pimix == NULL) {
		pr_err(" pimix==NULL len=%d, assign default value 170\n", len);
		ptim_rac_val_avg = 170; /* update by SPM before enter LP scenario */
	} else {
		pr_err(" pimix=%d\n", *pimix);
	...
	g_uvlo_status = pmic_get_register_value(PMIC_UVLO_RSTB_STATUS);
...
#ifdef CONFIG_PMIC_OVP_DISABLE
	pmic_read_interface(0xF48, &rVal, 0xffff, 0x0);
	rVal &= ~0x1;
	pmic_config_interface(0xF48, rVal, 0xffff, 0x0);
	pmic_read_interface(0xF48, &rVal, 0xffff, 0x0);
	PMICLOG("PMIC OVP:0xF48=0x%x\n", rVal);
#endif
#ifdef CONFIG_PMIC_BATTERY_THERMAL_DISABLE
	pmic_read_interface(0xF9E, &rVal, 0xffff, 0x0);
	rVal &= ~0x1;
	pmic_config_interface(0xF9E, rVal, 0xffff, 0x0);
	pmic_read_interface(0xF9E, &rVal, 0xffff, 0x0);
	PMICLOG("PMIC BATTERY THERMAL:0xF9E=0x%x\n", rVal);
#endif
