...
static void s2mu005_init_regs(struct s2mu005_fuelgauge_data *fuelgauge)
{
	...
	if(por_state & 0x10) {
		...
		dev_info(&fuelgauge->i2c->dev, "%s: temp dumpdone\n", __func__);
...
//		np = of_find_compatible_node(NULL, NULL, "samsung,sec-battery");
		if (!np) {
			pr_err("%s np NULL\n", __func__);
		} else {
...
pr_info("%s: fuelgauge->pdata->fuelgauge_name = %s\n",
		__func__,fuelgauge->pdata->fuelgauge_name);
...
pr_info("%s: S2MU005 Fuelgauge battery_paramsnode get\n", __func__);
			/* get battery_table */
				...
				const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		pr_info("%s: S2MU005 Fuelgauge dev.of_node\n", __func__);
		ret = s2mu005_fuelgauge_parse_dt(fuelgauge);
		if (ret < 0)
			goto err_parse_dt;
	} else {
		pr_info("%s: S2MU005 Fuelgauge else dev.of_node\n", __func__);
