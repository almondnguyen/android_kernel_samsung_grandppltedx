...
{
#if defined(CONFIG_MTK_HP_ANASWITCH)
	pr_warn("HP_Ana_Switch_to_Release (+)\n");
	usleep_range(10000, 11000);
...
static void Apply_Speaker_Gain(void)
{
	...
	cur_spk_gain = (Ana_Get_Reg(SPK_ANA_CON0) & 0x7800) >> 11;
	...
	if (index > oldindex) {
		...
		while (offset > 0) {
			Ana_Set_Reg(SPK_ANA_CON0, (oldindex + count) << 11,
				    0x7800);
...
void OpenAnalogTrimHardware(bool enable)
{
	if (enable) {
		...
		udelay(50);
		Ana_Set_Reg(ZCD_CON2, 0x0408, 0xffff);
		/* Set HPR/HPL gain as 0dB */
...
#if 1
void OpenAnalogSpeaker(bool bEnable)
{
	pr_warn("OpenAnalogSpeaker bEnable = %d", bEnable);
	if (bEnable) {
		SetHplTrimOffset(2048);
		SetHprTrimOffset(2048);
		mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] = 44100;
...
void setIntSpkGainZero(void)
{
	...
	Ana_Set_Reg(SPK_ANA_CON0, 0x1 << 11, 0x7800); /* restore to 1: 0dB */
...
void setIntSpkGainToIndex(int index)
{
	...
	Ana_Set_Reg(SPK_ANA_CON0, index << 11, 0x7800); /* set to index */
}
...
static void SetHpLOffsetTrim(void)
{
	...
	pr_debug("%s mHplTrimOffset = %d TrimOffset = %d\n", __func__, mHplTrimOffset, TrimOffset);
...
static void SetHpLSpkOffsetTrim(void)
{
	...
	pr_warn("%s OffsetTrimming = %d, (mHplSpkTrimOffset(%d)- TrimOffset(%d))\n", __func__,
...
static void SetHpRSpkOffsetTrim(void)
{
	...
	pr_warn("%s OffsetTrimming = %d, (mHprSpkTrimOffset(%d)- TrimOffset(%d))\n", __func__,
...
static void HeadsetVoloumeRestore(int target_index)
{
	...
	Ana_Set_Reg(ZCD_CON2, (target_index << 7) | target_index, 0xf9f); /* restore to target_index */
...
static void HeadsetVoloumeSet(int original_index)
{
	int index = 0, oldindex = 0, offset = 0, count = 1;

	index = mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTR];
	oldindex = original_index;
	pr_warn("%s index = %d oldindex = %d\n", __func__ , index, oldindex);
	if (index > oldindex) {
		offset = index - oldindex;
		while (offset > 0) {
...
static void Audio_Amp_Change(int channels, bool enable)
{
	...
	if (enable) {
		...
		if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] == false
			...
			HeadsetVoloumeSet(HP_TRIM_INDEX); /* 8: 0dB */
		...
		if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HEADSETL] == false
			...
			HeadsetVoloumeRestore(HP_TRIM_INDEX); /* 8: 0dB, Set HPR/HPL gain as -1dB, step by step */
...
static void Speaker_Amp_Change(bool enable)
{
	if (enable) {
		...
		/* Set IV buffer MUX select open/open */
...
static void Headset_Speaker_Amp_Change(bool enable)
{
	...
	if (enable) {
		...
		/* apply volume setting */
		HeadsetVoloumeSet(SPKHP_TRIM_INDEX); /* original is 18: -10dB */
		Apply_Speaker_Gain();
		...
		HeadsetVoloumeRestore(SPKHP_TRIM_INDEX);
		/* 18: -10dB, Set HPR/HPL gain to trimmed level, step by step */
	...
	EnableDcCompensation(false); /* Need to disable dc offset(applied in IV buf) before closing spk amp */
...
static int Headset_PGAL_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	oldindex = mCodec_data->mAudio_Ana_Volume[AUDIO_ANALOG_VOLUME_HPOUTL];
	pr_warn("%s index = %d oldindex = %d\n", __func__, index, oldindex);
	if (index > oldindex) {
		offset = index - oldindex;
		while (offset > 0) {
			Ana_Set_Reg(ZCD_CON2, (oldindex + count), 0x1f);
			offset--;
			count++;
			udelay(200);
		}
	...
	Ana_Set_Reg(ZCD_CON2, index, 0x1f); /* restore to target_index */
...
static int Headset_PGAR_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (index > oldindex) {
		offset = index - oldindex;
		while (offset > 0) {
			Ana_Set_Reg(ZCD_CON2, ((oldindex + count) << 7),0xf80);
			offset--;
			count++;
			udelay(200);
		}
	...
	Ana_Set_Reg(ZCD_CON2, index << 7, 0xf80); /* restore to target_index */
...
static int Codec_ADC_SampleRate_Get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
...
static int Codec_ADC_SampleRate_Set(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
...
static int Codec_DAC_SampleRate_Get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
...
static int Codec_DAC_SampleRate_Set(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
...
static const struct snd_kcontrol_new mt6331_snd_controls[] = {
	...
	SOC_SINGLE_EXT("Codec_ADC_SampleRate", SND_SOC_NOPM, 0, SOC_NORMAL_USE_RATE_MAX, 0, Codec_ADC_SampleRate_Get,
		       Codec_ADC_SampleRate_Set),
	SOC_SINGLE_EXT("Codec_DAC_SampleRate", SND_SOC_NOPM, 0, SOC_NORMAL_USE_RATE_MAX, 0, Codec_DAC_SampleRate_Get,
		       Codec_DAC_SampleRate_Set),
};
...
static bool TurnOnADcPowerACC(int ADCType, bool enable)
{
	...
	if (enable) {
		...
		pr_warn("%s: SRate_VUL1 = %d\n", __func__, SampleRate_VUL1);
		/* uint32 SampleRate_VUL2 = mBlockSampleRate[AUDIO_ANALOG_DEVICE_IN_ADC_2]; */
		if (GetMicbias == 0) {
			MicbiasRef = Ana_Get_Reg(AUDENC_ANA_CON9) & 0x0780;
			/* save current micbias ref set by accdet */
			pr_warn("%s: MicbiasRef = %#x\n", __func__, MicbiasRef);
			GetMicbias = 1;
		}
		if (GetAdcStatus() == false) {
			audckbufEnable(true);
		...
		if (ADCType == AUDIO_ANALOG_DEVICE_IN_ADC1) {	/* main and headset mic */
			...
			if (mCodec_data->mAudio_Ana_Mux[AUDIO_MICSOURCE_MUX_IN_1] == 0) {
				...
				Ana_Set_Reg(AUDENC_ANA_CON9, 0x0701, 0xff19);
				/* Enable MICBIAS0, MISBIAS0 = 1P9V,
				...
				/* micbias1 DCCopuleNP */
				Ana_Set_Reg(AUDENC_ANA_CON9, 0x0710, 0xff10);
	...
	pr_warn("%s: AUDENC_ANA_CON9 = %#x\n", __func__, Ana_Get_Reg(AUDENC_ANA_CON9));
...
{
	uint32 enable = ucontrol->value.integer.value[0];
	pr_info("%s() enable = %d, mAudio_Analog_Mic1_mode = %d\n",
		__func__, enable, mAudio_Analog_Mic1_mode);
